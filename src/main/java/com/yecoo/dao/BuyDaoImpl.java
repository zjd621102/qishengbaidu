...
public class BuyDaoImpl extends BaseDaoImpl {
	...
	public List<CodeTableForm> getBuyList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT username FROM suser WHERE userid = t.maker) makername, (SELECT btypename FROM sbtype WHERE btype = t.btype) btypename,"
				+ " (SELECT SUM(sum) FROM bbuyrow WHERE buyid = t.buyid) allsum FROM bbuy t WHERE 1 = 1";
		String cond = getBuyListCondition(form);
		sql  += cond;
	...
	public CodeTableForm getBuyById(int buyid, HttpServletRequest request) {
		...
		String sql = "SELECT a.*, (SELECT username FROM suser WHERE userid = a.maker) makername, (SELECT btypename FROM sbtype WHERE btype = a.btype) btypename"
		...
		try {
			...
			if(iReturn >= 1 && currflow.equals("结束")) { //流程结束
				...
				StringBuffer sql = new StringBuffer("INSERT INTO bpay(btype, maker, paydate, relateno, relatemoney,")
					...
					.append("', buydate, buyno, (SELECT SUM(sum) INTO isum FROM bbuyrow WHERE buyid = buyid), '申请', '").append(createdate)
