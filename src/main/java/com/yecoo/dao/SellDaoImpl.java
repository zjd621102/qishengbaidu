...
public class SellDaoImpl extends BaseDaoImpl {
	...
	public List<CodeTableForm> getSellList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT username FROM suser WHERE userid = t.maker) makername, (SELECT manuname FROM smanu WHERE manuid = t.manuid) manuname,"
				+ " (SELECT SUM(realsum) FROM bsellrow WHERE sellid = t.sellid) allrealsum FROM bsell t WHERE 1 = 1";
		String cond = getSellListCondition(form);
		sql  += cond;
	...
	public CodeTableForm getSellById(int sellid, HttpServletRequest request) {
		...
		String sql = "SELECT a.*, (SELECT username FROM suser WHERE userid = a.maker) makername, (SELECT manuname FROM smanu WHERE manuid = a.manuid) manuname"
				+ " FROM bsell a WHERE a.sellid = '" + sellid + "'";
		CodeTableForm codeTableForm = dbUtils.getFormBySql(sql);
		
		sql = "SELECT a.*, b.productno, (SELECT unitname FROM cunit WHERE unitid = a.unit) unitname"
		...
		try {
			...
			if(iReturn >= 1 && currflow.equals("结束")) { //流程结束
				...
				StringBuffer sql = new StringBuffer("INSERT INTO bpay(btype, maker, paydate, relateno, relatemoney,")
					...
					.append("', selldate, sellno, (SELECT SUM(realsum) FROM bsellrow WHERE sellid = sellid), '申请', '").append(createdate)
				...
				if(iReturn >= 1) { //生成销售单
					...
					sql.append("INSERT INTO bpayrow(payid, manuid, manubankname, manubankcardno, manuaccountname, plansum, realsum)")
						...
						.append(" (SELECT SUM(realsum) FROM bsellrow WHERE sellid = t.sellid), (SELECT SUM(realsum) FROM bsellrow WHERE sellid = t.sellid)")
