...
public class PayDaoImpl extends BaseDaoImpl {
	...
	public List<CodeTableForm> getPayList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT btypename FROM sbtype WHERE btype = t.btype) btypename, (SELECT username FROM suser WHERE userid = t.maker) makername,"
	...
	public CodeTableForm getPayById(int payid, HttpServletRequest request) {
		...
		String sql = "SELECT a.*, (SELECT btypename FROM sbtype WHERE btype = a.btype) btypename, (SELECT username FROM suser WHERE userid = a.maker) makername"
				+ " FROM bpay a WHERE a.payid = '" + payid + "'";
		CodeTableForm codeTableForm = dbUtils.getFormBySql(sql);
		
		sql = "SELECT a.*, (SELECT manuname FROM smanu WHERE manuid = a.manuid) manuname FROM bpayrow a WHERE a.payid = '" + payid + "'";
