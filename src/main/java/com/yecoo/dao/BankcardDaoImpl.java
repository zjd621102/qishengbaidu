...
public class BankcardDaoImpl extends BaseDaoImpl {
	...
	public List<CodeTableForm> getBankcardList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT banktypename FROM cbanktype WHERE banktypeid = t.banktype) banktypename,"
				+ " (SELECT statusname FROM cstatus WHERE statusid = t.status) statusname FROM sbankcard t WHERE 1 = 1";
		String cond = getBankcardListCondition(form);
		sql  += cond;
	...
	public CodeTableForm getBankcardById(int bankcardid) {
		...
		String sql = "SELECT a.*, (SELECT banktypename FROM cbanktype WHERE banktypeid = a.banktype) banktypename,"
				+ " (SELECT statusname FROM cstatus WHERE statusid = a.status) FROM sbankcard a WHERE a.bankcardid = '" + bankcardid + "'";
	...
	public List<CodeTableForm> getTransferaccountList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT bankcardno FROM sbankcard WHERE bankcardid = t.bankcardid) bankcardno,"
				+ " (SELECT bankcardno FROM sbankcard WHERE bankcardid = t.transferbankcardid) transferbankcardno FROM btransferaccount t WHERE 1 = 1";
		String cond = getTransferaccountListCondition(form);
		sql += cond;
	...
	public List<CodeTableForm> getReceandpayList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT bankcardno INTO vbankcardno FROM sbankcard WHERE bankcardid = t.bankcardid) bankcardno,"
				+ " (SELECT receandpaytypename FROM creceandpaytype WHERE receandpaytypeid = t.receandpaytype) receandpaytypename FROM breceandpay t WHERE 1 = 1";
		String cond = getReceandpayListCondition(form);
		sql += cond;
		sql += " ORDER BY t.createtime DESC";
		sql += " LIMIT " + (pageNum-1)*numPerPage + "," + numPerPage;
	...
	public List<CodeTableForm> getTransactionList(CodeTableForm form) {
		...
		String sql = "SELECT b.bankcardno, b.realsum*if(a.btype='SKD', 1, -1) realsum, a.paydate, a.payid, a.relateno,"
				+ " (SELECT btypename FROM sbtype WHERE btype = a.btype) btypename"
