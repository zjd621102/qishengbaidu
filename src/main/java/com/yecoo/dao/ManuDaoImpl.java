...
public class ManuDaoImpl extends BaseDaoImpl {
	...
	public List<CodeTableForm> getManuList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT manutypename FROM cmanutype WHERE manutypeid = t.manutypeid) manutypename ,"
				+ " (SELECT statusname FROM cstatus WHERE statusid = t.statusid) statusname,"
