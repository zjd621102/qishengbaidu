...
//			month = DateUtils.getAdjustTime(DateUtils.getNowDateTime(), "month", "", 1);// 默认为上个月
		...
		String sql = "SELECT t.*, (SELECT staffstatusname FROM cstaffstatus WHERE staffstatusid = t.staffstatus) staffstatusname,"
				+ " (SELECT stafftypename FROM cstafftype WHERE stafftypeid = t.stafftype) stafftypename,"
				+ " (SELECT IFNULL(SUM(a.salary), 0) FROM bworkrow a, bwork b WHERE a.workid = b.workid AND b.staffid = t.staffid AND b.workmonth = '" + month + "') monthsalary FROM sstaff t WHERE 1 = 1";
	...
	public CodeTableForm getStaffById(int staffid) {
		...
		String sql = "SELECT a.*, (SELECT staffstatusname FROM cstaffstatus WHERE staffstatusid = a.staffstatus) staffstatusname,"
				+ " (SELECT stafftypename FROM cstafftype WHERE stafftypeid = a.stafftype) stafftypename FROM sstaff a WHERE a.staffid = '" + staffid + "'";
