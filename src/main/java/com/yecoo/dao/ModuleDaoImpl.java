...
public class ModuleDaoImpl extends BaseDaoImpl {
	...
	public List<CodeTableForm> getModuleList() {
		...
		String sql = "SELECT t.*, (SELECT modulename FROM smodule WHERE moduleid = t.parentid) parentname FROM smodule t WHERE t.parentid = '1' ORDER BY t.priority, t.moduleid";
	    try {
	...
	private List<CodeTableForm> getChildrenList(CodeTableForm form) {
	    ...
	    List<CodeTableForm> moduleList = new ArrayList<CodeTableForm>();
	    CodeTableForm codeTableForm = null;
		String sql = "SELECT t.*, (SELECT modulename FROM smodule WHERE moduleid = t.parentid) parentname FROM smodule t WHERE t.parentid = '" + form.getValue("moduleid")
	...
	public List<CodeTableForm> getModuleList(CodeTableForm form) {
		...
		String sql = "SELECT t.*, (SELECT modulename FROM smodule WHERE moduleid = t.parentid) parentname FROM smodule t WHERE 1 = 1";
		String cond = getModuleListCondition(form);
		sql  += cond;
	...
	public CodeTableForm getModuleById(int moduleid) {
		...
		String sql = "SELECT t.*, (SELECT modulename FROM smodule WHERE moduleid = t.parentid) parentname FROM smodule t WHERE t.moduleid = '"
				+ moduleid + "'";
